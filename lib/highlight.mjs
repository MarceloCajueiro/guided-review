...
export function escapeHtml(s) {
  return String(s)
    ...
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
    // Not strictly needed while every attribute here is double-quoted, but that
    // is a convention across dozens of interpolation sites, not an enforced one.
    .replace(/'/g, '&#39;');
}
...
// over keywords, or a keyword inside a string gets colored. The mechanism is
// that every rule is joined into one alternation, and JS alternation is
// first-match-wins — so array order *is* precedence. Reordering these for
// tidiness silently changes the output.
...
export function highlight(line, lang) {
  ...
  // The string rules backtrack badly on pathological input — 5,000 escaped
  // quotes on one line costs ~90ms. Generated files are usually collapsed
  // before reaching here, but "usually" is not a guarantee, and no line this
  // long is being read by a human anyway.
  if (line.length > 2000) return escapeHtml(line);
  if (!RULE_CACHE.has(lang)) {
    ...
    // Map each capture-group slot back to the rule that owns it. Rules are free
    // to contain groups of their own — several do — so slot N is not rule N,
    // and assuming it was silently emitted class="" for every rule declared
    // after the first one with an inner group.
    const owner = [];
    rules.forEach(([re], i) => {
      // The wrapper group, plus however many the rule declares itself. Counted
      // by compiling an always-matching pattern and reading its group count.
      const inner = new RegExp(`${re.source}|`).exec('').length - 1;
      for (let n = 0; n <= inner; n++) owner.push(i);
    });
    RULE_CACHE.set(lang, {
  ...
  const { re, classes, owner } = RULE_CACHE.get(lang);
  re.lastIndex = 0;
