...
function inline(s) {
  ...
  return escaped
    ...
    // Single `*` is emphasis, per Markdown \u2014 `**` is bold and ran above, so what
    // reaches here is either real emphasis or a bare asterisk.
    //
    // The opener must be followed by a word character, not just any non-space:
    // `*.rb` and `*/` are globs and comment markers, not the start of emphasis.
    // The closer must be preceded by one for the same reason. Allowing
    // punctuation on either side made `glob *.rb e outro *.js` italicise the
    // span between two unrelated globs \u2014 the pattern found an opener and a
    // closer that were never a pair. Emphasis spanning a line break is not
    // supported either: an unclosed `*` would otherwise run to the next
    // paragraph's asterisk.
    .replace(/(^|\s)\*([^\s*][^*\n]*[^\s*]|[^\s*])\*(?![\w*])/g, '$1<em>$2</em>')
...
// Four corners pointing outward — the conventional "expand to fill" affordance.
const EXPAND_ICON = '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M6 1.5H1.5V6M10 1.5h4.5V6M6 14.5H1.5V10M10 14.5h4.5V10" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>';
 ...
 */
// The rows of a diff table. Shared by the inline panel and the fullscreen
// overlay so the two cannot drift: the overlay's whole promise is that it shows
// the same file, just without the focus cut.
function diffRows(hunks, { lang, marks, idScope, focused }) {
  const rows = [];
...
// `L` is required, not defaulted: the locale tables are declared at the bottom
// of this file, so a default of `EN` here would read as safe while depending on
// render() never being called during module evaluation. Every caller has an `L`
// to hand anyway.
function renderFile(file, { marks = new Set(), open = true, idScope = '', prUrl = '#', L }) {
  ...
  const collapsed = !file.isBinary && total > 0 &&
    (file.isGenerated || (total > MAX_LINES && marks.size === 0));
  ...
  if (file.isBinary) {
    ...
    body = `<div class="elide">${why} ` +
      `<button type="button" class="link-btn js-full">${escapeHtml(L.fullscreen)}</button>` +
      ` · <a href="${escapeHtml(prUrl)}">GitHub</a></div>`;
  } else {
    const hunks = focused ? focusHunks(file, marks) : file.hunks;
    const rows = diffRows(hunks, { lang, marks, idScope, focused });
    body = `<div class="code"><table>${rows}</table></div>`;
    if (focused) {
      // The escape hatch is now local: the same file, unfocused, over the whole
      // viewport. Leaving GitHub as the only way to read the rest meant the
      // reader lost the narrative to see the code it was describing.
      body += `<div class="elide">${escapeHtml(L.focusNote)} ` +
        `<button type="button" class="link-btn js-full">${escapeHtml(L.fullscreen)}</button>` +
        ` · <a href="${escapeHtml(prUrl)}">GitHub</a></div>`;
    }
  }

  // The overlay's copy of the file: every hunk, no focus cut, no marks dropped.
  // Carried in a <template> so it costs no layout until opened, and no ids so it
  // cannot collide with the panel that owns them.
  //
  // Built here rather than in the page script because the script has no access
  // to the parsed diff — reconstructing the hidden lines from the DOM is not
  // possible once focusHunks has dropped them.
  //
  // Only files the panel actually truncates get one. A file rendered whole has
  // nothing more to show, and a button that opens an identical view — or worse,
  // one whose template is missing and silently does nothing — is a promise the
  // page cannot keep.
  const truncated = !file.isBinary && total > 0 && (focused || collapsed);
  let full = '';
  if (truncated) {
    const rows = diffRows(file.hunks, { lang, marks, idScope: '', focused: false });
    full = `<template class="full-src"><table>${rows}</table></template>`;
  }

  // Rendered inside <summary>, which toggles the panel on click — the handler
  // stops propagation so opening the overlay does not also collapse the file
  // behind it.
  const fullBtn = truncated
    ? `<button type="button" class="full-btn js-full" title="${escapeHtml(L.fullscreen)}" ` +
      `aria-label="${escapeHtml(L.fullscreen)}">${EXPAND_ICON}</button>`
    : '';
  const head =
    `<summary>${CARET}${pathHtml(file.path)}${fileTag(file)}${counts}${fullBtn}</summary>`;

  const isOpen = open && !file.isGenerated ? ' open' : '';
  return `<details class="file" data-path="${escapeHtml(file.path)}"${isOpen}>` +
    `${head}${body}${full}</details>`;
}
...
function renderBeat(beat, byPath, used, L, { key, prUrl }) {
  const anchors = Array.isArray(beat.files) ? beat.files : [];
  const perFile = marksFor(anchors, byPath);

  const prose = `<div class="beat-prose">${paragraphs(beat.text)}${renderNotes(beat.notes, L.notes)}</div>`;

  const files = [];
  for (const [path, marks] of perFile) {
    const file = byPath.get(path);
    used.add(path);
    files.push(renderFile(file, { marks, idScope: `${key}-L${slugPath(path)}`, prUrl, L }));
  }
...
function renderChapter(ch, n, byPath, used, L, prUrl) {
  const beats = (ch.beats || [])
    .map((b, i) => renderBeat(b, byPath, used, L, { key: `c${n}b${i + 1}`, prUrl }))
    .join('');
...
(function () {
  ...
  //
  // The panel shows the anchored passage; this shows the same file whole. The
  // rows come from a <template> the renderer emitted next to the panel, because
  // the lines focusHunks dropped are not in the DOM to be recovered.
  var overlay = document.getElementById('overlay');
  var overlayBody = document.getElementById('overlay-body');
  var overlayPath = document.getElementById('overlay-path');
  // Where focus came from, so closing returns it there instead of dumping the
  // keyboard user back at the top of the document.
  var opener = null;

  function openFull(details, btn) {
    var tpl = details.querySelector('.full-src');
    var table = tpl && tpl.content.firstElementChild;
    if (!table) return;
    opener = btn || null;
    overlayPath.textContent = details.getAttribute('data-path') || '';
    overlayBody.textContent = '';
    overlayBody.appendChild(table.cloneNode(true));
  ...
  function closeFull() {
    ...
    // Dropping the rows keeps a 5,000-line file from sitting in the document
    // for the rest of the session.
    overlayBody.textContent = '';
    if (opener) { opener.focus(); opener = null; }
  }

  // Delegated: panels are static, but binding per button would mean one listener
  // per file on a large PR.
  addEventListener('click', function (e) {
    var btn = e.target.closest && e.target.closest('.js-full');
    if (!btn) return;
    var details = btn.closest('.file');
    if (!details) return;
    // The header button lives inside <summary>, which toggles the panel on
    // click. Without this the file collapses behind the overlay.
    e.preventDefault();
    e.stopPropagation();
    openFull(details, btn);
  });

  document.getElementById('overlay-close').addEventListener('click', closeFull);
  // Clicking the backdrop closes; clicking the code inside must not.
  ...
  addEventListener('keydown', function (e) {
    ...
    // Esc closes the overlay; every other binding is suspended while it is up,
    // so j/k cannot scroll the page behind it.
...
const EN = {
  ...
  legendAdd: 'added',
  legendDel: 'removed',
  legendMark: 'the lines this paragraph explains',
  focusNote: 'Showing the passage this section is about —',
  fullscreen: 'View the whole file',
  closeFull: 'Close',
  fullHint: 'Full file — every line, nothing hidden.',
  notes: NOTE_LABELS,
};
...
const PT = {
  ...
  legendAdd: 'adicionado',
  legendDel: 'removido',
  legendMark: 'as linhas que este parágrafo explica',
  focusNote: 'Mostrando a passagem de que esta seção trata —',
  fullscreen: 'Ver o arquivo inteiro',
  closeFull: 'Fechar',
  fullHint: 'Arquivo completo — todas as linhas, nada escondido.',
...
export function render(story, files, { lang = 'en' } = {}) {
  ...
  const appendix = leftover.length
    ? `<section class="appendix"><h3>${escapeHtml(L.appendixTitle)}</h3><p>${escapeHtml(L.appendixBody)}</p>` +
      leftover.map((f) => renderFile(f, { open: false, prUrl, L })).join('') +
      '</section>'
    : '';
  ...
  // The amber swatch is only worth explaining when an anchor actually renders
  // one. A story whose beats cite whole files highlights nothing, and a legend
  // for a colour absent from the page sends the reader looking for it.
  const hasMarks = (story.chapters || []).some((ch) =>
    (ch.beats || []).some((b) =>
      (b.files || []).some((spec) => {
        const { path, from } = parseRange(spec);
        return from != null && byPath.has(path);
      })
    )
  );
  const legend =
    `<div class="legend">` +
    `<span class="l-add"><i></i><b>${escapeHtml(L.legendAdd)}</b></span>` +
    `<span class="l-del"><i></i><b>${escapeHtml(L.legendDel)}</b></span>` +
    (hasMarks
      ? `<span class="l-mark"><i></i><b>${escapeHtml(L.legendMark)}</b></span>`
      : '') +
    `</div>`;
...
<div class="overlay" id="overlay" role="dialog" aria-modal="true" aria-labelledby="overlay-path">
  <div class="overlay-head">
    <span class="path" id="overlay-path"></span>
    <span class="hint">${escapeHtml(L.fullHint)}</span>
    <span class="spacer"></span>
    <button class="overlay-close" id="overlay-close" type="button">
      <kbd>Esc</kbd>${escapeHtml(L.closeFull)}
    </button>
  </div>
